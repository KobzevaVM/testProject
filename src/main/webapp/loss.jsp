...
<body>
    <div class="header">
        <h1>
            <script>
                let goodAnswer = "<%= ((PlayerInfo) request.getSession().getAttribute("playerInfo")).getGoodAnswer() %>"
                if(goodAnswer == 0) {
                    document.write("Ты отклонил вызов.");
                }
                if(goodAnswer == 1) {
                    document.write("Ты не пошлел на преговоры.");
                }
                if(goodAnswer == 2) {
                    document.write("Твою ложь разоблачили.");
                }
            </script>
            <br><br>
            <b>
                <script>
                    document.write("ПОРАЖЕНИЕ.")
                </script>
            </b>
        </h1>
    </div>
